...
export const bstChecker = root => {
  ...
  function assert(root, condition) {
    ...
    invariant(!right || value < right.value, 'Value on RHS is less than root');

    assert(left, v => {
      invariant(v <= value, 'LHS child is greater than root node');
    });

    assert(right, v => {
      invariant(v > value, 'RHS child is less than root node');
    });
  }
